...
async def create_category(
    ...
    category.branch_id = current_user.get("branch_id")
    return menu_service.create_category(category)
...
async def update_category(
    ...
    if not db_category:
        raise HTTPException(status_code=404, detail="Category not found")
    if db_category.tenant_id != current_user["tenant_id"]:
        raise HTTPException(status_code=403, detail="Access denied")
    
    updated_category = menu_service.update_category(category_id, category)
...
async def create_item(
    ...
    return menu_service.create_item(item)
...
async def update_item(
    ...
    updated_item = menu_service.update_item(item_id, item)
    if not updated_item:
        raise HTTPException(status_code=404, detail="Item not found")
    return updated_item
...
async def create_variant(
    ...
    """
    menu_service = MenuService(db)
    # In a full implementation, we'd need to add variant methods to the service
    # For now, we'll work directly with the repository
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    from app.models import MenuVariant
    db_variant = MenuVariant(**variant.dict())
    db_variant.tenant_id = current_user["tenant_id"]
    db_variant.branch_id = current_user.get("branch_id")
...
async def read_variant(
    ...
    """
    Retrieve a specific menu variant by ID.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    ...
    # Check tenant access
    if variant.tenant_id != current_user["tenant_id"]:
        raise HTTPException(status_code=403, detail="Access denied")
    return variant
...
async def update_variant(
    ...
    """
    Update a specific menu variant.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    # Check tenant access
    ...
    if not db_variant:
        raise HTTPException(status_code=404, detail="Variant not found")
    if db_variant.tenant_id != current_user["tenant_id"]:
        raise HTTPException(status_code=403, detail="Access denied")
    
    updated_variant = repo.update_variant(variant_id, variant)
...
async def delete_variant(
    ...
    """
    Delete a specific menu variant.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    # Check tenant access
...
async def create_modifier_group(
    ...
    db_modifier_group = MenuModifierGroup(**modifier_group.dict())
    db_modifier_group.tenant_id = current_user["tenant_id"]
    db_modifier_group.branch_id = current_user.get("branch_id")
...
async def read_modifier_groups(
    ...
    """
    Retrieve menu modifier groups.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    modifier_groups = repo.get_modifier_groups(
        tenant_id=current_user["tenant_id"],
        menu_item_id=menu_item_id,
        skip=skip,
        limit=limit
...
async def read_modifier_group(
    ...
    """
    Retrieve a specific menu modifier group by ID.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    ...
    # Check tenant access
    if modifier_group.tenant_id != current_user["tenant_id"]:
        raise HTTPException(status_code=403, detail="Access denied")
...
async def update_modifier_group(
    ...
    """
    Update a specific menu modifier group.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    # Check tenant access
    ...
    if not db_modifier_group:
        raise HTTPException(status_code=404, detail="Modifier group not found")
    if db_modifier_group.tenant_id != current_user["tenant_id"]:
        raise HTTPException(status_code=403, detail="Access denied")
    
    updated_modifier_group = repo.update_modifier_group(group_id, modifier_group)
...
async def delete_modifier_group(
    ...
    """
    Delete a specific menu modifier group.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    # Check tenant access
...
async def create_modifier(
    ...
    db_modifier = MenuModifier(**modifier.dict())
    db_modifier.tenant_id = current_user["tenant_id"]
    db_modifier.branch_id = current_user.get("branch_id")
...
async def read_modifier(
    ...
    """
    Retrieve a specific menu modifier by ID.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    ...
    # Check tenant access
    if modifier.tenant_id != current_user["tenant_id"]:
        raise HTTPException(status_code=403, detail="Access denied")
    return modifier
...
async def update_modifier(
    ...
    """
    Update a specific menu modifier.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    # Check tenant access
    ...
    if not db_modifier:
        raise HTTPException(status_code=404, detail="Modifier not found")
    if db_modifier.tenant_id != current_user["tenant_id"]:
        raise HTTPException(status_code=403, detail="Access denied")
    
    updated_modifier = repo.update_modifier(modifier_id, modifier)
...
async def delete_modifier(
    ...
    """
    Delete a specific menu modifier.
    """
    from app.modules.menu.repository import MenuRepository
    repo = MenuRepository(db)
    # Check tenant access
